...
export function createWebSocketHandler(): WebSocketHandler<WebSocketData> {
    ...
    return {
        ...
        message(ws: AppWebSocket, message: string | Buffer) {
            ...
            const messageType = baseMessage.data.header.type;
            ...
            switch (messageType) {
                ...
                default: {
                    logger.warn({ type: messageType }, "Unknown message type");
                    handleWebSocketError(ws, ErrorCode.INVALID_MESSAGE, `Unknown message type: ${messageType}`);
                }
            }
        },
